...
1. 事件 $X\_{1}$ : 得到 10000 元，概率为 20%
2. 事件 $X\_{2}$ : 得到 100 元， 概率为 80%
...
1. $S$ 上加法满足交换和结合律
2. $S$ 存在单位元 $0\_{S}$ ,每个元素 $a$ 存在逆元 $−a$
3. $1\_{F} ⋅ a = a, p ⋅ 0\_{S} = 0\_{S}$
...
设 S 为 F 上的线性空间，$\beta = {b\_{1},\cdots,b\_{m\}}$ 为 S 的一个子集，若存在不全为 0 的 $p\_{1}, \cdots p\_{m} \in F$ 使得 $p\_{1}b\_{1} + \cdots + p\_{m}b\_{m} = 0$,则称 $\beta$ 线性相关。否则就是线性无关。
